...
```
        ┌──────────────────────────── PC ────────────────────────────┐
        │  tio /dev/ttyACM0            run_sim.py --serial /dev/ttyUSB0│
        │   (콘솔 · printk)              (데이터 링크 · 프레임)         │
        └───────┬───────────────────────────────┬─────────────────────┘
                │ USB (콘솔+플래시)              │ USB
                │                                │
        ┌───────┴────────┐              ┌────────┴─────────┐
        │  ST-Link VCP   │              │  USB-UART 어댑터  │
        │  (보드 내장)    │              │  (FT232RL, 3.3V) │
        └───────┬────────┘              └────────┬─────────┘
...
```
 FT232RL(3.3V)                STM32F746G-DISCO (Arduino 헤더)
 ┌─────────┐
 │ GND ────┼──────────────── GND
